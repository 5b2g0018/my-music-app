...
function App() {
    ...
    const [isDiaryLocked, setIsDiaryLocked] = useState(false);
    const [diaryPassword, setDiaryPassword] = useState('');
    const [unlockedDiaryIds, setUnlockedDiaryIds] = useState([]); // 👈 就是漏了這行，下面才會崩潰！
    ...
    const fetchUserDiaries = async (email) => {
        try {
            const q = query(collection(db, 'diaries'), where('userEmail', '==', email));
            const querySnapshot = await getDocs(q);
            const diariesList = []; // 先用 any[] 承接，避免型別嚴格比對衝突
    ...
    const moodCounts = myDiaries.reduce((acc, item) => {
    ...
    const renderNavbar = () => {
        ...
        <ul style={{
          ...
          <li style={{ whiteSpace: 'nowrap' }}><a href="#" onClick={(e) => { e.preventDefault(); if (!loggedInUser) {
            alert('請先登入！');
            setCurrentView('login');
        }
        else {
            setCurrentView('capsule');
        } }} style={{ textDecoration: 'none', color: 'var(--accent)', fontWeight: 'bold', fontSize: '14px' }}>📬 時光膠囊</a></li>
          <li style={{ whiteSpace: 'nowrap' }}><a href="#" onClick={(e) => { e.preventDefault(); if (!loggedInUser) {
            alert('請先登入！');
            setCurrentView('login');
        }
          ...
          <li style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '4px', whiteSpace: 'nowrap' }}>
            ...
            <select value={theme} onChange={(e) => setTheme(e.target.value)} style={{ padding: '4px 8px', borderRadius: '12px', border: '1px solid var(--border)', background: 'var(--bg-sec)', color: 'var(--text-main)', fontWeight: 'bold', cursor: 'pointer', outline: 'none', fontSize: '13px' }}>
          ...
          {loggedInUser ? (<li style={{ display: 'flex', alignItems: 'center', gap: '12px', marginLeft: '8px', whiteSpace: 'nowrap' }}>
            ...
            </li>) : (<li style={{ marginLeft: '8px', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ maxWidth: '700px', margin: '40px auto', padding: '40px', background: 'var(--bg-color)', border: '1px solid var(--border)', borderRadius: '16px', position: 'relative' }}>
          ...
          <div style={{
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontWeight: 'bold', color: 'var(--text-main)', fontSize: '16px' }}>
              <input type="checkbox" checked={isDiaryLocked} onChange={(e) => {
                setIsDiaryLocked(e.target.checked);
                if (!e.target.checked)
                    setDiaryPassword(''); // 取消勾選時自動幫你把殘留密碼清空
            ...
            {/* 🔑 當打勾上鎖時，才會優雅地展開密碼輸入框 */}
            {isDiaryLocked && (<div style={{ marginTop: '15px' }}>
                <p style={{ margin: '0 0 8px 0', fontSize: '14px', color: 'var(--text-sub)' }}>
                  請設定此篇日記的專屬解鎖暗號：
                </p>
                <input type="password" placeholder={theme === 'gd' ? "請輸入此篇權志龍狂放詩篇的專屬暗號..." :
                    theme === 'ive' ? "請輸入解鎖大千金精緻生活誌的璀璨密碼..." :
                        theme === 'babymonster' ? "請輸入闖入怪獸禁區的重低音密鑰..." :
                            "請輸入此篇日記的專屬解鎖密碼..."} value={diaryPassword} onChange={(e) => setDiaryPassword(e.target.value)} style={{
                    width: '100%',
                    padding: '12px 14px',
                    borderRadius: '8px',
                    border: '1px solid var(--border)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '25px' }}>
              ...
              <textarea rows={7} value={futureLetter} onChange={(e) => setFutureLetter(e.target.value)} placeholder="不論是夢想，還是悄悄話..." style={{
                width: '100%',
                padding: '16px',
                borderRadius: '14px',
                border: isDark ? '1px solid rgba(255,255,255,0.15)' : '1px solid var(--border)',
            ...
            {/* 封存按鈕 */}
            <button onClick={handleSaveCapsule} style={{
                width: '100%',
                padding: '15px',
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              ...
              <div style={{
                ...
                {Object.entries(moodCounts).map(([mood, count]) => (<div key={mood} style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
    ...
    if (currentView === 'register' || currentView === 'login') {
        ...
        <div style={{ maxWidth: '400px', margin: '80px auto', padding: '40px', background: 'var(--bg-color)', border: '1px solid var(--border)', borderRadius: '16px' }}>
          {currentView === 'register' ? (<form onSubmit={handleRegisterSubmit}>
              <h2>建立您的帳號</h2>
            ...
            </form>) : (<form onSubmit={handleLoginSubmit}>
              <h2>歡迎回來</h2>
    ...
    return (<>
      ...
      <section className="hero" style={{
        ...
        <div className="eyebrow" style={{
          ...
          {theme === 'gd' ? ("🌼 COUP D'ETAT！G-DRAGON 潮流藝術脈絡同步中") : theme === 'ive' ? ("✨ WHAT'S AFTER LIKE? IVE 閃耀大千金秀台已就位") : theme === 'babymonster' ? ("👹 ATTENTION！BABYMONSTER 怪物新人重磅音浪突襲") : theme === 'aespa' ? ("🪐 Welcome to KWANGYA！aespa 虛擬同步網絡已對接") : theme === 'blackpink' ? ("🖤 BLACKPINK IN YOUR AREA！BLINK 烈焰重低音模組已啟動") : theme === 'kpop' ? ("🍭 ONE IN A MILLION！TWICE 9人全員應援板已聯動") : ("📜 MEMOIR SYSTEM ｜ 經典暖米時光溫柔載入中，靜候你的日常篇章")}
        </div>
        ...
        <p className="hero-sub fade-up visible" style={{
          ...
          {theme === 'gd' ? ("「八九不離十，我是唯一的潮流」— 踏入 GD 的黑金高街實驗室，像揮灑噴漆一般，留住你最特立獨行、不可一世的靈魂印記。") : theme === 'ive' ? ("「我就像鑽石，無懈可擊」— 啟動張員瑛式的美學視角，讓你的日常日記充斥精緻蝴蝶結與奢華光芒，你就是生活的 C 位大千金。") : theme === 'babymonster' ? ("「天生怪獸，天生狠角色」— 承襲美式硬核嘻哈的大勢力量，點燃你體內不服輸的怪物基因，在這裡寫下燃炸全場的新人傳奇。") : theme === 'aespa' ? ("「打破邊界，定義未來」— 穿越現實與虛擬交錯的世界，在這裡記錄每一次突破自我、每一次勇敢升級，寫下專專屬於你的未來篇章。") : theme === 'blackpink' ? ("「頂峰相見，無人能擋」— 帶上你的粉紅應援氣球槌，在這裡刻下最具野心、最不服輸且充滿光芒的震撼瞬間。") : theme === 'kpop' ? ("「只要我們在一起，就是 ONE IN A MILLION」— 點亮甜蜜的 Candy Bong 應援光芒，在這裡用滿滿的愛與元氣，記錄下九位女孩與你最珍貴的閃耀瞬間。") : ("「留住歲月裡的溫柔」— 靜下心來，把平凡的日常慢慢鋪陳，在這裡將每一段恬靜的時光寫成最精緻、最值得細細品味的溫暖詩篇。")}
        </p>

        {/* 🚀 【就是這裡！】動作按鈕區塊，乖乖回歸原本的位置囉 */}
        <div className="hero-actions" style={{ display: 'flex', gap: '15px', marginTop: '20px', zIndex: 2 }}>
          {/* 主要按鈕：開始寫日記 */}
          <a href="#" className="btn-hero btn-hero-primary" style={{
            background: theme === 'gd' ? '#ffeb3b' :
                theme === 'ive' ? '#ff4081' :
          ...
          {/* 次要按鈕：看年度回顧 */}
          <a href="#" className="btn-hero btn-hero-secondary" style={{
            background: theme === 'gd' || theme === 'ive' || theme === 'babymonster' || theme === 'aespa' || theme === 'blackpink'
                ? '#000'
                : 'var(--bg-sec)',
            color: theme === 'gd' ? '#ffeb3b' :
                theme === 'ive' ? '#ff4081' :
                    theme === 'babymonster' ? '#ff1744' :
                        theme === 'aespa' ? '#00ffff' :
                            theme === 'blackpink' ? '#ff007f' :
                                'var(--text-main)',
            border: theme === 'gd' ? '1px solid #ffeb3b' :
        ...
        {(() => {
            try {
                ...
                if (unlockedCapsules.length === 0)
                    return null;
                return unlockedCapsules.map((capsule) => (<div key={capsule.id} style={{
      ...
      <section style={{ padding: '80px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {myDiaries.map((diary) => {
                ...
                return (<div key={diary.id} style={{ background: 'var(--bg-color)', padding: '28px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: '0 4px 20px rgba(0,0,0,0.01)', marginBottom: '20px' }}>
